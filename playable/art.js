...
const blades=bladePixels.map((b,i)=>{const r=crops[i];return [380+(b[0]-r[4])*.88,360+(b[1]-r[5])*.88,380+(b[2]-r[4])*.88,360+(b[3]-r[5])*.88];});
...
function draw(i,t=starts[i]+durations[i]*.5){f=i;g.imageSmoothingEnabled=false;const [x,y,w,h,ax,ay]=crops[i];g.drawImage(sprite,x,y,w,h,380+(x-ax)*.88,360+(y-ay)*.88,w*.88,h*.88);trails(t);}
return(time,idle=false)=>{let f=0,left=time;while(f<11&&left>=durations[f])left-=durations[f++];draw(idle?11:f,idle?-1000:time);};};
...
function draw(k,f,t=0){const i=k*4+f,[x,y,w,h,ax,ay]=rects[i],scale=.8,dx=k===2?[0,20,45,60][f]:k===0?[0,8,22,0][f]:0;g.imageSmoothingEnabled=false;g.save();g.translate(340+dx,356);if(f===2&&k<2){g.beginPath();const bx=k===0?800:770,edge=k===0?1135:1115,sy=k===0?192:540,sh=k===0?26:29;g.rect((bx-ax)*scale,(y-ay)*scale,(edge-bx)*scale,h*scale);g.rect((edge-ax)*scale,(sy-ay)*scale,(x+w-edge)*scale,sh*scale);g.clip();}g.drawImage(sprite,x,y,w,h,(x-ax)*scale,(y-ay)*scale,w*scale,h*scale);g.restore();
if(f===2){g.save();g.globalCompositeOperation='lighter';g.shadowColor='#ffc957';g.shadowBlur=10;g.strokeStyle='#fff4c2';g.lineWidth=2;g.beginPath();if(k===0){g.moveTo(381,231);g.lineTo(498,231);}else if(k===1){g.ellipse(353,272,107,18,-.06,.15,2.6);}else{g.moveTo(402,243);g.lineTo(499,329);}g.stroke();g.restore();}}
return(k,time,idle=false)=>{let f=0,left=time;while(f<3&&left>=times[k][f])left-=times[k][f++];draw(k,idle?0:f,time);};};
