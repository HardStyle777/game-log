...
for(let i=0;i<n;i++){const j=i*4;if(bg[i]){d[j+3]=0;continue;}const x=i%w,y=Math.floor(i/w);let distance=5;for(let dy=-4;dy<=4;dy++)for(let dx=-4;dx<=4;dx++){if(x+dx>=0&&x+dx<w&&y+dy>=0&&y+dy<h&&bg[i+dy*w+dx])distance=Math.min(distance,Math.max(Math.abs(dx),Math.abs(dy)));}if(distance>4)continue;const low=Math.min(d[j],d[j+1],d[j+2]),high=Math.max(d[j],d[j+1],d[j+2]),neutral=high-low<28;const floors=[0,75,105,145,180],floor=floors[distance];if(low<=floor||(!neutral&&distance>2))continue;const a=Math.min(1,(255-low)/(255-floor));for(let c=0;c<3;c++)d[j+c]=Math.max(0,Math.min(255,(d[j+c]-255*(1-a))/Math.max(.01,a)));d[j+3]=Math.round(d[j+3]*a);}
};
