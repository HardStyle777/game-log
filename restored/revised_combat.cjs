...
function battle({player:p,enemies,state={},seed=1,maxSeconds=180}){
 ...
 let s=skills.find(s=>s.ready<=t+1e-8&&(s.type==='potion'?potions>0&&potionReady<=t&&hp/maxHp<=clamp(s.hpBelow??.4):cp>=Math.max(0,s.cost||0)));
 if(!s)s={id:'basic',mult:1,cast:1,targets:1,basic:true};
 ...
 pending={s,done:t+chaseDelay+Math.max(.05,(s.cast||.1)*(s.type==='potion'?1:slow/Math.max(.1,p.attackSpeed||1)))};chaseDelay=0;
 }
 if(hp<=0)return out(false,'defeat');if(!living().length)return out(true,'victory');begin();
 ...
 if(!living().length)return out(true,'victory');
 if(pending.done<=t+1e-8){const s=pending.s;if(s&&!disabled(ps)){
 casts[s.id]=(casts[s.id]||0)+1;
 ...
 const hit=p.absoluteHit?1:clamp((p.hit??.95)-(p.ignoreEvade?0:Math.max(0,(e.evade||0)-(active(e.statuses,'lightDebuff')?.15:0)))-(active(ps,'blind')||active(ps,'lightDebuff')?.15:0),.05,1);if(rnd()>=hit)continue;
 ...
 const raw=Math.max(0,element==='physical'?(p.atk||0)+flat:(p.magicPower||0))*(s.mult??1)*(crit?Math.max(1,p.critMult||1.5):1)*(active(ps,'weaponBreak')?.7:1)*(active(ps,'berserk')?1.2:1);
 let damage=mitigate(raw,element,e,e.statuses);
 for(const [el,v]of Object.entries(p.elementDamage||{})){const amt=(v[0]+rnd()*(v[1]-v[0]))*(1-resist(e.resist,el));if(el==='earth'){const duration=Math.max(.1,p.poisonDuration||2);e.poisonRate=amt/duration*(1-clamp((e.statusResist?.poison||0)+(e.statusResist?.state||0)+(e.statusResist?.all||0)));e.poisonUntil=t+duration;}else damage+=amt;}
 const actual=Math.min(e.hp,Math.max(0,damage));e.hp-=actual;if(actual>0)delete e.statuses.sleep;hp=Math.min(maxHp,hp+actual*clamp(p.life));proc(p.procs,e);if(p.crush&&rnd()<clamp(p.crush)*(1-clamp(e.crushResist)))e.hp-=Math.max(0,e.hp)*.25;
 ...
 const absorb=clamp(p.absorb?.[e.element]);hp-=d*(1-absorb);if(d>0)delete ps.sleep;if(hp>0)hp=Math.min(maxHp,hp+d*absorb);
 ...
 if(!living().length)return out(true,'victory');if(!pending)begin();
 }
